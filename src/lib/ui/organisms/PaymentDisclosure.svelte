<script lang="ts">
	// THE MOMENT MONEY MOVES, said out loud. Deliberately ABOVE the button, in
	// lowercase, in the plain register the product uses when it has something
	// real to admit. It is not a fine-print block and it is not collapsed behind
	// a link.
	//
	// EVERY CLAUSE HERE IS LOAD-BEARING AND EVERY ONE OF THEM IS CHECKED AGAINST
	// CODE. The mapping lives in docs/pro-payments.md, clause by clause, with the
	// file and line that makes each true. If a change makes one of these
	// sentences false, the sentence is not the thing to edit.
</script>

<section aria-labelledby="money-heading" class="mt-8 rounded-lg border border-line bg-ink-raised px-5 py-5">
