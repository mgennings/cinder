...
- **Pricing is moving to prepaid credits**, and the conversion is designed but not built. `docs/pro-payments.md` carries the decision that a credit is spent at the **mint** (not at create, finalize, or claim), that the grant's nonce is the idempotency key that makes a retried send free, that a destroyed transfer spends its credit and why Cinder structurally cannot refund it, and why a remaining balance must never travel inside a grant.
