...
try {
	...
	// A production build leaves prerendered HTML with its production CSP under
	// .svelte-kit. Vite dev will serve those stale pages unchanged, so the phone
	// UI loads while Safari refuses every local API. They are generated bytes;
	// remove them before the review server derives a fresh CSP from this run.
	rmSync(resolve(ROOT, '.svelte-kit/output/prerendered'), { recursive: true, force: true });
